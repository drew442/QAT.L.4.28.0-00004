...
#define HAL_CLR_BIT(wrd, bit) ((wrd) & ~(1U << (bit)))
#define HAL_SET_BIT(wrd, bit) ((wrd) | 1U << (bit))
			    ...
			    unsigned char ae, unsigned char mode)
{
	...
	new_csr = (mode == 4) ?
		HAL_SET_BIT(csr, CE_INUSE_CONTEXTS_BITPOS) :
		HAL_CLR_BIT(csr, CE_INUSE_CONTEXTS_BITPOS);
			   ...
			   unsigned char ae, unsigned char mode)
{
	...
	new_csr = (mode) ?
		HAL_SET_BIT(csr, CE_NN_MODE_BITPOS) :
		HAL_CLR_BIT(csr, CE_NN_MODE_BITPOS);
			   ...
			   unsigned char mode)
{
	...
	case ICP_LMEM0:
		new_csr = (mode) ?
			HAL_SET_BIT(csr, CE_LMADDR_0_GLOBAL_BITPOS) :
			HAL_CLR_BIT(csr, CE_LMADDR_0_GLOBAL_BITPOS);
		break;
	case ICP_LMEM1:
		new_csr = (mode) ?
			HAL_SET_BIT(csr, CE_LMADDR_1_GLOBAL_BITPOS) :
			HAL_CLR_BIT(csr, CE_LMADDR_1_GLOBAL_BITPOS);
		break;
	case ICP_LMEM2:
		new_csr = (mode) ?
			HAL_SET_BIT(csr, CE_LMADDR_2_GLOBAL_BITPOS) :
			HAL_CLR_BIT(csr, CE_LMADDR_2_GLOBAL_BITPOS);
		break;
	case ICP_LMEM3:
		new_csr = (mode) ?
			HAL_SET_BIT(csr, CE_LMADDR_3_GLOBAL_BITPOS) :
			HAL_CLR_BIT(csr, CE_LMADDR_3_GLOBAL_BITPOS);
		break;
	default:
				...
				unsigned char mode)
{
	...
	csr &= IGNORE_W1C_MASK;
	new_csr = (mode) ?
		  HAL_SET_BIT(csr, CE_T_INDEX_GLOBAL_BITPOS) :
		  HAL_CLR_BIT(csr, CE_T_INDEX_GLOBAL_BITPOS);
	if (new_csr != csr)
		qat_hal_wr_ae_csr(handle, ae, CTX_ENABLES, new_csr);
}
			     ...
			     unsigned char mode)
{
	...
	new_csr = (mode) ?
		  HAL_SET_BIT(csr, MMC_SHARE_CS_BITPOS) :
		  HAL_CLR_BIT(csr, MMC_SHARE_CS_BITPOS);
	if (new_csr != csr)
				   ...
				   unsigned int *endpc)
{
	...
	qat_hal_rd_ae_csr(handle, ae, AE_MISC_CONTROL, &csr_val);
	scs_flag = csr_val & (1 << MMC_SHARE_CS_BITPOS);
	newcsr_val = HAL_CLR_BIT(csr_val, MMC_SHARE_CS_BITPOS);
	...
	qat_hal_rd_ae_csr(handle, ae, AE_MISC_CONTROL, &csr_val);
	newcsr_val = scs_flag ? HAL_SET_BIT(csr_val, MMC_SHARE_CS_BITPOS) :
				HAL_CLR_BIT(csr_val, MMC_SHARE_CS_BITPOS);
	qat_hal_wr_ae_csr(handle, ae, AE_MISC_CONTROL, newcsr_val);
