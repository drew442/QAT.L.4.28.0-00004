				     ...
				     unsigned int num_in_bytes)
{
	...
	if (num_in_bytes > ICP_QAT_CSS_MMP_MAX_UCODE_LEN) {
		pr_err("QAT: error, mmp size overflow %d\n", num_in_bytes);
		return -EINVAL;
	}
	if (!num_in_bytes || num_in_bytes % 4) {
		pr_err("QAT: error, mmp size error %d\n", num_in_bytes);
		return -EINVAL;
	}
	while (num_in_bytes) {
		memcpy(&outval, ptr, 4);
				...
				struct icp_qat_css_fw_size css_fw_size)
{
	...
	unsigned int ae_mode_init_size = 0;
	struct icp_qat_css_hdr *css_hdr = (struct icp_qat_css_hdr *)image;

	if ((css_hdr->header_len * css_dword_size) != css_fw_size.ae_img_offset) {
		pr_err("QAT: error, input fw_type %u image fw size error\n", fw_type);
		goto err;
	}
	if ((css_hdr->size * css_dword_size) != size) {
		pr_err("QAT: error, input fw_type %u image size error\n", fw_type);
		goto err;
	}
	if (fw_type != css_hdr->fw_type) {
		pr_err("QAT: error, input fw type error, expected %u, got %u\n",
		       fw_type, css_hdr->fw_type);
		goto err;
	}
	if (size <= css_fw_size.ae_img_offset) {
		pr_err("QAT: error, input fw_type %u image size too small %lu\n",
		       fw_type, size);
		goto err;
	}
	size -= css_fw_size.ae_img_offset;

	if (fw_type == CSS_AE_FIRMWARE) {
		ae_mode_init_size = sizeof(struct icp_qat_simg_ae_mode) +
			ICP_QAT_SIMG_AE_INIT_SEQ_LEN;
		if (size < ae_mode_init_size) {
			pr_err("QAT: error, input AE image size too small %lu\n",
			       size);
			goto err;
		}
		size -= ae_mode_init_size;
		if (size > ICP_QAT_CSS_AE_MAX_UCODE_LEN) {
			pr_err("QAT: error, input AE image size overflow %lu\n", size);
			goto err;
		}
	} else if (fw_type == CSS_MMP_FIRMWARE) {
		if (size > ICP_QAT_CSS_MMP_MAX_UCODE_LEN) {
			pr_err("QAT: error, input MMP image size overflow %lu\n",
			       size);
			goto err;
		}
	}
				...
				int fw_type)
{
	...
	qat_uclo_check_css_fw_size(handle, &css_fw_size);
	if (qat_uclo_check_image(handle, image, size, fw_type, css_fw_size))
		return -EINVAL;
...
static int qat_uclo_wr_uof_img(struct icp_qat_fw_loader_handle *handle)
{
	...
	for (i = 0; i < obj_handle->uimage_num; i++) {
		if (!obj_handle->ae_uimage[i].img_ptr)
			return -EINVAL;
		if (obj_handle->ae_uimage[i].uwords_num >
				handle->hal_handle->max_ustore) {
			pr_err("QAT: error, input image size overflow %d\n",
			       obj_handle->ae_uimage[i].uwords_num);
			return -EINVAL;
		}
