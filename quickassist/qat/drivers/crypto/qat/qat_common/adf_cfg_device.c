...
void adf_cfg_get_accel_algo_cap(struct adf_accel_dev *accel_dev)
{
	...
	u32 accel_capabilities_mask = hw_data->accel_capabilities_mask;
...
static int adf_cfg_update_pf_accel_cap_mask(struct adf_accel_dev *accel_dev)
{
	...
	/*
	 * Check for sym capability in extended_dc_capabilities before
	 * disabling FW undefined sym capabilities
	 */
	if (hw_data->extended_dc_capabilities &&
	    !(hw_data->extended_dc_capabilities &
