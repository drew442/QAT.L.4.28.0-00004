...
If older manual QAT modules under `/lib/modules/$(uname -r)/updates/drivers/crypto/qat` would shadow `/updates/dkms`, the installer archives them under `/lib/modules/$(uname -r)/updates/qat-dkms-shadowed-*`, runs `depmod`, and verifies every managed module resolves to `/updates/dkms`.
